...
class TopicOpAdminController < ::ApplicationController
  ...
  def update_topic_status
    ...
    return render_fail "topic_op_admin.not_enabled", status: 405 unless SiteSetting.topic_op_admin_enabled
    ...
    when "closed"
      unless guardian.can_close_topic_as_op?(topic)
        TopicOpUserAdminBot.botLogger("@#{guardian.user.username} 试图在未经授权的情况下" + (enabled ? '关闭' : '打开') + "话题 #{topic.url}")
        return render_fail "topic_op_admin.no_perm", status: 403
      end
      TopicOpUserAdminBot.botLogger("@#{guardian.user.username} " + (enabled ? '关闭' : '打开') + "话题 #{topic.url}")
    ...
    when "visible"
      ...
      TopicOpUserAdminBot.botLogger("@#{guardian.user.username} " + (enabled ? '显示' : '隐藏') + "话题 #{topic.url}")
    ...
    topic.update_status(status, enabled, TopicOpUserAdminBot.getBot(), until: params[:until], message: "由楼主的要求操作")
  ...
  def update_slow_mode
    ...
    TopicOpUserAdminBot.botLogger("@#{guardian.user.username} 对话题 #{topic.url} #{(enabled ? '设置了' : '禁用了')}慢速模式")
  ...
  def set_topic_op_admin_status
    ...
    unless guardian.can_manipulate_topic_op_adminable?
      TopicOpUserAdminBot.botLogger("@#{guardian.user.username} 在未经授权的情况下试图修订话题 #{topic.url} 楼主管理模式：\n```\n#{params.to_json}\n```")
      return render_fail "topic_op_admin.no_perm", status: 403
    end
    ...
    TopicOpUserAdminBot.botLogger("@#{guardian.user.username} 修订话题 #{topic.url} 楼主管理模式：\n```\n#{params.to_json}\n```")

    render json:
             success_json.merge!(
              topic_op_admin_status_update: params[:new_status]
             )
  end
  ...
  def request_for_topic_op_admin
    ...
    TopicOpUserAdminBot.botLogger("@#{guardian.user.username} 申请将话题 #{topic.url} 设为楼主管理模式：\n[quote=\"#{guardian.user.username}\"]\n#{params[:raw]}\n[/quote]")

    render json:
             success_json.merge!(
              message: I18n.t("topic_op_admin.get_request")
             )
  end
